...
double ORIFICE_COEFF = 0.167;
double FREE_WEIR_COEFF = 0.54;
double SUBMERGED_WEIR_COEFF = 0.056;
double g = 9.80665;
...
double DLLEXPORT   swmm_getNodeLinkageFlow(int index){
    return Node[index].linkage_flow * FOOT;
}
...
void DLLEXPORT   swmm_setNodeLinkageFlow(int index, double linkage_flow){
    Node[index].linkage_flow = linkage_flow; 
}
...
nodeData* DLLEXPORT   swmm_getNodeData(int index)
{
	if ( IsOpenFlag )
	{
		...
		data->inflow = Node[index].inflow * FOOT3;
		data->outflow = Node[index].outflow * FOOT3;
		data->head = (Node[index].invertElev + Node[index].newDepth) * FOOT;
		data->crestElev = (Node[index].invertElev + Node[index].fullDepth) * FOOT;
		data->type = Node[index].type;
		data->subIndex = Node[index].subIndex;
		data->invertElev = Node[index].invertElev * FOOT;
		data->initDepth = Node[index].initDepth * FOOT;
		data->fullDepth = Node[index].fullDepth * FOOT;
		data->surDepth = Node[index].surDepth * FOOT;
		data->pondedArea = Node[index].pondedArea * FOOT2;
		data->degree = Node[index].degree;
		data->updated = Node[index].updated;
		data->crownElev = Node[index].crownElev * FOOT;
		data->losses = Node[index].losses * FOOT3;
		data->newVolume = Node[index].newVolume * FOOT3;
		data->fullVolume = Node[index].fullVolume * FOOT3;
		data->overflow = Node[index].overflow * FOOT3;
		data->newDepth = Node[index].newDepth * FOOT;
		data->newLatFlow = Node[index].newLatFlow * FOOT3;
		return data;
	}
	return NULL;
}
...
int DLLEXPORT  get_linkage_type(double wse, double crest_elev,
    ...
    double depth_2d = wse - crest_elev;
    double weir_ratio = overflow_area / weir_width;
    bool overflow = node_head > wse;
    bool drainage = node_head < wse;

    bool free_weir = drainage && (node_head < crest_elev);
    bool submerged_weir = drainage && (node_head > crest_elev) && (depth_2d < weir_ratio);
    bool drainage_orifice = drainage && (node_head > crest_elev) && (depth_2d > weir_ratio);

    bool overflow_orifice = overflow;
...
double DLLEXPORT get_linkage_flow(double wse, double node_head, double weir_width,
    ...
    double head_up = MAX(wse, node_head);
    double head_down = MIN(wse, node_head);
    double head_diff = head_up - head_down;
    double upstream_depth = head_up - crest_elev;
...
double DLLEXPORT get_overflow_area(int index, double node_depth){
    ...
    double surf_area = node_getSurfArea(index, node_depth);
    if(surf_area <= 0.0){
...
double DLLEXPORT solve_dt(){
    double old = NewRoutingTime / 1000;
    double new = OldRoutingTime / 1000;
    double _dt = new - old;
    if (_dt <= 0.0){
...
double DLLEXPORT apply_linkage_flow(int index, double h, double z, double cell_surf){
    ...
    double wse = z + h;
    double crest_elev = 0;
    double node_crest_elev = (Node[index].invertElev + Node[index].fullDepth);
    if(node_crest_elev != z){
        double full_depth = z - Node[index].invertElev;
    ...
    //## linkage type ##
    double overflow_area = get_overflow_area(index, Node[index].newDepth);
    //# weir width is the circumference (node considered circular)
    double weir_width = PI * 2.0 * sqrt(overflow_area / PI);
    //# determine linkage type
    double head = (Node[index].invertElev + Node[index].newDepth);
    int linkage_type = get_linkage_type(wse, crest_elev, head, weir_width, overflow_area);

    //## linkage flow ##
    //int index, double wse, double node_head, double weir_width, double crest_elev, int linkage_type, double overflow_area
    double new_linkage_flow = get_linkage_flow(wse, head, weir_width,
 ...
 /**
    ...
    */
    ...
    bool overflow_to_drainage = Node[index].linkage_flow > 0 && new_linkage_flow < 0;
    bool drainage_to_overflow = Node[index].linkage_flow < 0 && new_linkage_flow > 0;
